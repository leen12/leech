...
MODEL = os.environ.get("LEECH_CLASSIFY_MODEL", "")

# Bodies are multi-line, so they can't ride the one-line format -- these go to
# the whole-file rewrite path instead. The ONLY hand-kept list of tool names.
NO_ONE_LINE = {"write_file", "append_file"}

# Tools whose `path` names something to be CREATED, so it can't already exist.
# For every other tool -- including any added later -- `path` must be an exact
# hit in the workspace listing. Fail closed.
CREATES = {"make_dir"}
...
# Derived from TOOLS, so a new tool shows up here and in the prompt for free.
...
def _parse(reply, files):
    ...
    rest = m.group(2)
    keys = SIGS[name]
    # split on ` key=` boundaries for the keys this tool actually accepts
    ...
    if not all(k in args for k in REQUIRED[name]):
        return None
    # `path` must be something that really exists; `new_path` is a destination,
    # so it only has to stay inside the workspace.
    p = args.get("path")
